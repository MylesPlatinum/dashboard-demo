...
def load_data():
    ...
    try:
        ...
        if debug_mode:
            st.write(f"Revenue section:")
            st.dataframe(revenue_section)
            st.write(f"Hours section:")
            st.dataframe(hours_section)
        
        # Dynamic column mapping based on number of branches
        ...
        for branch, col_idx in col_mapping.items():
            for i, period in enumerate(periods):
                if period and period.strip() and period != 'None':
                    ...
                    # Skip periods where both revenue and hours are NaN or 0
                    rev_numeric = pd.to_numeric(rev_val, errors='coerce')
                    hrs_numeric = pd.to_numeric(hrs_val, errors='coerce')
                    
                    # Only add data if at least one value exists
        ...
        costs_raw = pd.read_excel(costs_path, header=0)
        
        if debug_mode:
            st.write("### Debug: Raw Costs Data")
            st.dataframe(costs_raw.head(20))
        
        # Build costs column list dynamically
